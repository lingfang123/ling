...
<div class="page-filter wrap">
  <div class="breadcrumbs"><a href="index/frontpage">首页</a>&gt;<span class="cur">投资列表</span></div>
            ...
            <dd>
              <ul>
                ...
                <c:forEach items="${marks}" var="mark">
                	<li class="n2"><a href="javascript:url('post_type','car');" id="post_type_car">${mark.markname}</a></li>
              	</c:forEach>
              </ul>
            </dd>
          </dl>
          ...
          <dl class="repayment">
            ...
            <dd>
              <ul>
                ...
                <li class="n2"><a id="repay_style_end" href="javascript:url('repay_style','end');">等额本息</a> </li>
                <li class="n2"><a id="repay_style_endmonth" href="javascript:url('repay_style','endmonth');">先息后本</a> </li>
              </ul>
            </dd>
          </dl>
        </div>
  ...
  <div class="invest-list mrt30 clearfix">
    <div class="hd">
      ...
      <div class="count">
        <ul>
          <li class="line">累计投资交易金额&nbsp;&nbsp;<span class="f20 bold">${sumMoney}万元</span></li>
    ...
    <div class="bd">
      <div class="invest-table clearfix">
        ...
        <!------------投资列表-------------->
        <c:forEach items="${markInfos.lists}" var="markinfo">
	        <div class="item">
	          <ul>
	            <li class="col-330 col-t"><a href='<c:if test="${markinfo.countmoney==markinfo.money}">markinfo/info?id=${markinfo.id}&markid=${markinfo.markid}</c:if><c:if test="${markinfo.countmoney<markinfo.money}">markinfo/info2?id=${markinfo.id}&markid=${markinfo.markid}</c:if>' target="_blank"><i class="icon icon-che" title="${mark.markname}"></i></a><a class="f18" href='<c:if test="${markinfo.countmoney==markinfo.money}">markinfo/info?id=${markinfo.id}&markid=${markinfo.markid}</c:if><c:if test="${markinfo.countmoney<markinfo.money}">markinfo/info2?id=${markinfo.id}&markid=${markinfo.markid}</c:if>' title="${markinfo.headline}" target="_blank"> ${markinfo.headline} </a></li>
	            ...
	            <li class="col-120-2"> 
		            <c:if test="${markinfo.countmoney==markinfo.money}">
               		...
               		<c:if test="${markinfo.struts==0}">
               			<a class="ui-btn btn-gray" href="">未开始</a>
               		</c:if>
               		<c:if test="${markinfo.struts==2}">
               			<a class="ui-btn btn-gray" href="">流标</a>
               		</c:if>
               		<c:if test="${markinfo.struts==6}">
               			<a class="ui-btn btn-gray" href="">已完成</a>
               		</c:if>
	            </li>
	          </ul>
	        </div>
      ...
      <div class="pagination clearfix mrt30">
      	<span class="page">
	      	<c:if test="${markInfos.currPage>1}">
		      	<a href="markinfo/markInfoes?currentPage=1">首页</a>
		    </c:if>
		    <c:if test="${markInfos.currPage>1}">
		      	<a href="markinfo/markInfoes?currentPage=${markInfos.currPage-1}">上一页</a>&nbsp;
		    </c:if>
		    <c:forEach var="i" begin="1" step="1" end="${markInfos.totalPage}">
		      <a <c:if test="${markInfos.currPage==i}">class="curr"</c:if> href="markinfo/markInfoes?currentPage=${i}">${i}</a>
		    </c:forEach>  
		    <c:if test="${markInfos.currPage<markInfos.totalPage}">
		      <a href="markinfo/markInfoes?currentPage=${markInfos.currPage+1}">下一页</a>
		    </c:if>
		    <c:if test="${markInfos.currPage<markInfos.totalPage}">
		      <a href="markinfo/markInfoes?currentPage=${markInfos.totalPage}">尾页</a>&nbsp;
		    </c:if>
		    <em>共${markInfos.totalPage}页&nbsp;</em>
	    </span>
